...
uint8_t Dasti_Pos_ch[]={6,0,1,2,3,4,5};
uint8_t Morgh_Pos_ch[]={4,6,7,8,9};
uint8_t MorghShakhdar_Pos_ch[]={8,6,7,8,9,2,10,11,5};
uint8_t Boghalamoon_Pos_ch[]={4,12,4,13,5};
uint8_t Belderchin_Pos_ch[]={3,14,15,5};
uint8_t Gharghavol_Pos_ch[]={3,14,16,17};
uint8_t Kabk_Pos_ch[]={6,18,7,19,2,20,21};
uint8_t Ordak_Pos_ch[]={2,22,21};
uint8_t Ghaz_Pos_ch[]={4,23,4,16,24};
uint8_t Shotormorgh_Pos_ch[]={4,34,35,36,37};
uint8_t Timer_Pos_ch[]={4,38,20,26,39};
uint8_t Charkhsh_Pos_ch[]={8,25,15,26,27,2,3,4,5};
uint8_t Dama_Pos_ch[]={3,28,29,24};
uint8_t Rotobat_Pos_ch[]={4,12,21,4,30};
uint8_t Zaman_Pos_ch[]={4,23,43,6,24};
uint8_t Rooz_Pos_ch[]={3,40,41,42};
uint8_t Dooreh_Pos_ch[]={4,44,6,45,24};
uint8_t Setter_Pos_ch[]={3,31,32,15};
uint8_t Hatcher_Pos_ch[]={3,33,7,15};

uint8_t custom_character[][8] = {
		   {0x00, 0x01, 0x09, 0x14, 0x05, 0x1f, 0x00, 0x00 },//0//Tah
		   {0x00, 0x01, 0x02, 0x04, 0x02, 0x1f, 0x18, 0x10 },//1//Kom
       {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },//2//space
		   {0x00, 0x00, 0x00, 0x19, 0x07, 0x18, 0x03, 0x00 },//3//Yad
       {0x00, 0x00, 0x00, 0x00, 0x06, 0x06, 0x02, 0x0c },//4//Oo
			 {0x00, 0x00, 0x00, 0x00, 0x00, 0x13, 0x12, 0x0C },//5//_Y
			 {0x00, 0x00, 0x00, 0x0c, 0x02, 0x01, 0x02, 0x0c },//6//D
       {0x00, 0x00, 0x00, 0x0E, 0x01, 0x1F, 0x00, 0x04 },//7//J_
       {0x00, 0x04, 0x04, 0x04, 0x04, 0x07, 0x00, 0x00 },//8//A_
       {0x00, 0x00, 0x0C, 0x02, 0x0E, 0x10, 0x12, 0x0C },//9//J
       {0x00, 0x00, 0x00, 0x03, 0x09, 0x1F, 0x00, 0x08 },//10//Hab
       {0x00, 0x02, 0x07, 0x00, 0x15, 0x1F, 0x00, 0x00},//11//_Sh_
       {0x00, 0x00, 0x00, 0x02, 0x02, 0x02, 0x04, 0x18 },//12//R
			 {0x00, 0x00, 0x00, 0x00, 0x00, 0x1f, 0x05, 0x07 },//13//m
       {0x00, 0x02, 0x00, 0x07, 0x05, 0x1F, 0x00, 0x00 },//14//F
       {0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x04, 0x18 },//15//_R		 			 
       {0x00, 0x02, 0x00, 0x02, 0x02, 0x02, 0x04, 0x18 },//16//Z
       {0x00, 0x04, 0x00, 0x11, 0x11, 0x11, 0x0e, 0x00 },//17//N
       {0x00, 0x00, 0x00, 0x0E, 0x01, 0x1F, 0x00, 0x00},//18//H
       {0x00, 0x02, 0x02, 0x02, 0x02, 0x13, 0x12, 0x0C},//19//L
       {0x00, 0x05, 0x00, 0x07, 0x05, 0x1f, 0x00, 0x00 },//20//Gh
       {0x04, 0x04, 0x04, 0x07, 0x05, 0x1f, 0x00, 0x00 },//21//t_			 			 
       {0x00, 0x00, 0x00, 0x02, 0x02, 0x1e, 0x00, 0x04 },//22//B
       {0x00, 0x04, 0x04, 0x04, 0x04, 0x04, 0x00,	0x00 },//23//A
       {0x00, 0x0A, 0x00, 0x04, 0x0A, 0x0A, 0x0E, 0x00},//24/H_t
       {0x00, 0x00, 0x03, 0x18, 0x05, 0x1F, 0x00, 0x00},//25/Tah
       {0x00, 0x00, 0x00, 0x02, 0x02, 0x1e, 0x00, 0x0c },//26//Y	
       {0x01, 0x02, 0x04, 0x02, 0x11, 0x1f, 0x00, 0x00 },//27//K
       {0x00, 0x00, 0x00, 0x06, 0x01, 0x0F, 0x08, 0x10},//28//Har
       {0x00, 0x00, 0x01, 0x01, 0x05, 0x05, 0x09, 0x10},//29//Ar
       {0x00, 0x0C, 0x00, 0x0C, 0x15, 0x1F, 0x00, 0x02},//30//Bat
       {0x00, 0x00, 0x00, 0x15, 0x15, 0x1f, 0x00, 0x00 },//31//S
       {0x00, 0x00, 0x18, 0x02, 0x02, 0x1f, 0x00, 0x00 },//32//_T_		 			 
       {0x00, 0x04, 0x0A, 0x0D, 0x09, 0x1E, 0x00, 0x00 },//33//H_	
       {0x00, 0x00, 0x02, 0x00, 0x01, 0x1F, 0x00, 0x00 },//34//N	
       {0x00, 0x10, 0x10, 0x17, 0x12, 0x0F, 0x00, 0x00 },//35//AA	
       {0x00, 0x00, 0x00, 0x00, 0x00, 0x1F, 0x05, 0x03},//36//M	
       {0x00, 0x06, 0x00, 0x02, 0x06, 0x0F, 0x00, 0x00 },//37//_Ht	
       {0x00, 0x00, 0x06, 0x00, 0x19, 0x1F, 0x08, 0x10 },//38//Too
       {0x00, 0x00, 0x0A, 0x00, 0x11, 0x1e, 0x00, 0x00 },//39//T	
       {0x00,0x00,0x15,0x15,0x15,0x0D,0x19,0x00 },//40//AlA			 
       {0x00,0x10,0x10,0x10,0x11,0x1F,0x00,0x06 },//41//ya			 
       {0x00,0x00,0x00,0x00,0x03,0x07,0x08,0x08},//42//M			 
       {0x00,0x01,0x01,0x01,0x01,0x1F,0x0C,0x0C},//43//Lam	
       {0x00,0x05,0x05,0x05,0x05,0x1D,0x00,0x00},//44//Al
       {0x00,0x00,0x00,0x00,0x03,0x0B,0x09,0x12},//45//Vr_oor
};
...
void LCD_putpersian(uint8_t StringCode,uint8_t posx,uint8_t posy,Alignment_t align)
{
	...
	{
		case MORGHSHAKHDAR_STR:
			lengthofcustom=MorghShakhdar_Pos_ch[0];
			posofcustomchar=&MorghShakhdar_Pos_ch[1];
		break;
		case BOGHALAMOON_STR:
			lengthofcustom=Boghalamoon_Pos_ch[0];
			posofcustomchar=&Boghalamoon_Pos_ch[1];
		break;
		case BELDERCHIN_STR:
			lengthofcustom=Belderchin_Pos_ch[0];
			posofcustomchar=&Belderchin_Pos_ch[1];
		break;
		case GHARGHAVOL_STR:
			lengthofcustom=Gharghavol_Pos_ch[0];
			posofcustomchar=&Gharghavol_Pos_ch[1];
		break;		
		case KABK_STR:
			lengthofcustom=Kabk_Pos_ch[0];
			posofcustomchar=&Kabk_Pos_ch[1];
		break;	
		case ORDAK_STR:
			lengthofcustom=Ordak_Pos_ch[0];
			posofcustomchar=&Ordak_Pos_ch[1];
		break;	
		case GHAZ_STR:
			lengthofcustom=Ghaz_Pos_ch[0];
			posofcustomchar=&Ghaz_Pos_ch[1];
		break;	
		case SHOTORMORGH_STR:
			lengthofcustom=Shotormorgh_Pos_ch[0];
			posofcustomchar=&Shotormorgh_Pos_ch[1];
		break;	
		case DASTI_STR:
			lengthofcustom=Dasti_Pos_ch[0];
			posofcustomchar=&Dasti_Pos_ch[1];
		break;
		case MORGH_STR:
			lengthofcustom=Morgh_Pos_ch[0];
			posofcustomchar=&Morgh_Pos_ch[1];
		break;
		case TIMER_STR:
			lengthofcustom=Timer_Pos_ch[0];
			posofcustomchar=&Timer_Pos_ch[1];
		break;
		case CHARKHSH_STR:
			lengthofcustom=Charkhsh_Pos_ch[0];
			posofcustomchar=&Charkhsh_Pos_ch[1];
		break;
		case DAMA_STR:
			lengthofcustom=Dama_Pos_ch[0];
			posofcustomchar=&Dama_Pos_ch[1];
		break;
		case ROTOBAT_STR:
			lengthofcustom=Rotobat_Pos_ch[0];
			posofcustomchar=&Rotobat_Pos_ch[1];
			memstartpos=Dama_Pos_ch[0];
		break;
		case ZAMAN_STR:
			lengthofcustom=Zaman_Pos_ch[0];
			posofcustomchar=&Zaman_Pos_ch[1];
		break;
		case ROOZ_STR:
			lengthofcustom=Rooz_Pos_ch[0];
			posofcustomchar=&Rooz_Pos_ch[1];
			memstartpos=Zaman_Pos_ch[0];
		break;
		case DOOREH_STR:
			lengthofcustom=Dooreh_Pos_ch[0];
			posofcustomchar=&Dooreh_Pos_ch[1];
		break;
		case SETTER_STR:
			lengthofcustom=Setter_Pos_ch[0];
			posofcustomchar=&Setter_Pos_ch[1];
			memstartpos=Dooreh_Pos_ch[0];
		break;
		case HATCHER_STR:
			lengthofcustom=Hatcher_Pos_ch[0];
			posofcustomchar=&Hatcher_Pos_ch[1];
			memstartpos=Dooreh_Pos_ch[0];
		break;
